...
// Знак "or" | позволяет нам указать в регулярном выражении сразу несколько возможных последовательностей совпадений. 


// Task 23
// Используйте знак |, чтобы исправить десятичное регулярное выражение, указанное выше, и получить такой результат:

let str23 = `0.011 .2 42 2.0 3.33 4.000 5 6 7.89012 0. .`;
let result23 = str23.match(/\d+\.\d* | \d*\.\d+/gm);
console.log(result23); // [ '0.011 ', ' 2.0', ' 3.33', ' 4.000', ' 7.89012', '0. ' ]


// Task 24
// Используйте знак |, классы символов, "optional" ? и т.д., чтобы создать одно регулярное выражение, соответствующее как целым числам, 
// так и числам с плавающей запятой (точкой), как обсуждалось в задаче в конце предыдущего шага (эта задачка немного посложнее, да ;))

let str24 = `42L 12 x 3.4f 6l 3.3 0F L F .2F 0.`;
let result24 = str24.match(/\d+\.*\w* | \d+\.?\w* | \d\.?\w*/gm);
// pattern: \d*\.\d+[fF]?|\d+\.\d*[fF]?|\d+[lLfF]?
console.log(result24); // ['42L ',   '12 ', ' 3.4f ', '6l ', '3.3 ',   '0F ', '2F ']


// круглые скобки () как группы захвата
// Проверяя, какой группе соответствует подстрока, мы можем сразу определить, является ли она значением float или значением long:
